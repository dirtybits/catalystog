...
crypto::Hash catalyst::proof_of_trust::get_hash() const {
	std::string s;
...
// P2pProtocolTypes
void ser(catalyst::UUID &v, seria::ISeria &s) { s.binary(&v, sizeof(v)); }

void ser_members(catalyst::PeerlistEntry &v, seria::ISeria &s) {
...
void ser_members(catalyst::NetworkAddress &v, seria::ISeria &s) {
	seria_kv("ip", v.ip, s);
	seria_kv("port", v.port, s);
}

void ser_members(catalyst::connection_entry &v, seria::ISeria &s) {
	seria_kv("adr", v.adr, s);
	seria_kv("id", v.id, s);
	seria_kv("is_income", v.is_income, s);
}

void ser_members(catalyst::CoreStatistics &v, seria::ISeria &s) {
...
void ser_members(catalyst::network_config &v, seria::ISeria &s) {
...
void ser_members(catalyst::basic_node_data &v, seria::ISeria &s) {
...
void ser_members(catalyst::CORE_SYNC_DATA &v, seria::ISeria &s) {
	if (s.is_input()) {
		uint32_t on_wire = 0;
...
void ser_members(catalyst::COMMAND_HANDSHAKE::request &v, seria::ISeria &s) {
	seria_kv("node_data", v.node_data, s);
	seria_kv("payload_data", v.payload_data, s);
}

void ser_members(catalyst::COMMAND_HANDSHAKE::response &v, seria::ISeria &s) {
	seria_kv("node_data", v.node_data, s);
	seria_kv("payload_data", v.payload_data, s);
	serialize_as_binary(v.local_peerlist, "local_peerlist", s);
}

void ser_members(catalyst::COMMAND_TIMED_SYNC::request &v, seria::ISeria &s) {
	seria_kv("payload_data", v.payload_data, s);
}
...
void ser_members(catalyst::COMMAND_REQUEST_STAT_INFO::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }
...
void ser_members(catalyst::COMMAND_REQUEST_NETWORK_STATE::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }

void ser_members(catalyst::COMMAND_REQUEST_NETWORK_STATE::response &v, seria::ISeria &s) {
...
void ser_members(catalyst::RawBlockLegacy &v, seria::ISeria &s) {
	...
	if (s.is_input()) {
		...
		std::transform(other_transactions.begin(), other_transactions.end(), std::back_inserter(v.transactions),
		    [](const std::string &s) { return catalyst::BinaryArray(s.data(), s.data() + s.size()); });
	} else {
		...
		std::transform(v.transactions.begin(), v.transactions.end(), std::back_inserter(other_transactions),
		    [](catalyst::BinaryArray &s) { return std::string(s.begin(), s.end()); });
		seria::seria_kv("block", other_block, s);
		seria::seria_kv("txs", other_transactions, s);
	}
}
...
void ser_members(catalyst::NOTIFY_NEW_TRANSACTIONS::request &v, seria::ISeria &s) {
	...
	if (s.is_input()) {
		...
		std::transform(v.txs.begin(), v.txs.end(), std::back_inserter(transactions),
		    [](const catalyst::BinaryArray &s) { return std::string(s.begin(), s.end()); });
		seria::seria_kv("txs", transactions, s);
	}
}
