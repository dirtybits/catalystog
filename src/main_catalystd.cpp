...
Options:
  ...
  --data-folder=<full-path>            Folder for blockchain, logs and peer DB [default: )" platform_DEFAULT_DATA_FOLDER_PATH_PREFIX
    R"(catalyst].
  --catalystd-authorization=<usr:pass> HTTP authorization for RPC.
...
int main(int argc, const char *argv[]) try {
	...
	if( !export_blocks.empty() && !backup_blockchain.empty() ){
		std::cout << "You can either export blocks or backup blockchain on one run of catalystd" << std::endl;
		return api::CATALYSTD_WRONG_ARGS;
	}
	...
	platform::ExclusiveLock coin_lock(coin_folder, "catalystd.lock");

	logging::LoggerManager log_manager;
	log_manager.configure_default(config.get_data_folder("logs"), "catalystd-");
