...
Node::Node(logging::ILogger &log, const Config &config, BlockChainState &block_chain)
	...
	if (!config.catalystd_bind_ip.empty() && config.catalystd_bind_port != 0)
		m_api.reset(new http::Server(config.catalystd_bind_ip, config.catalystd_bind_port,
	...
	m_commit_timer.once(DB_COMMIT_PERIOD_CATALYSTD);
	advance_long_poll();
}
...
void Node::sync_transactions(P2PClientCatalyst *who) {
...
void Node::P2PClientCatalyst::on_msg_bytes(size_t, size_t) {  // downloaded. uploaded
	//    node->peers.on_peer_bytes(get_address(), downloaded, uploaded,
	//    node->p2p.get_local_time());
}

CORE_SYNC_DATA
Node::P2PClientCatalyst::get_sync_data() const {
...
std::vector<PeerlistEntry> Node::P2PClientCatalyst::get_peers_to_share() const {
	auto result =
	    m_node->m_peer_db.get_peerlist_to_p2p(m_node->m_p2p.get_local_time(), config.p2p_default_peers_in_handshake);
	return result;
}

void Node::P2PClientCatalyst::on_first_message_after_handshake() {
...
void Node::P2PClientCatalyst::on_msg_handshake(COMMAND_HANDSHAKE::request &&req) {
	NetworkAddress addr;
...
void Node::P2PClientCatalyst::on_msg_handshake(COMMAND_HANDSHAKE::response &&req) {
...
void Node::P2PClientCatalyst::on_msg_notify_request_chain(NOTIFY_RESPONSE_CHAIN_ENTRY::request &&req) {
	m_node->m_downloader.on_msg_notify_request_chain(this, req);
}

void Node::P2PClientCatalyst::on_msg_notify_request_objects(NOTIFY_REQUEST_GET_OBJECTS::request &&req) {
...
void Node::P2PClientCatalyst::on_msg_notify_request_objects(NOTIFY_RESPONSE_GET_OBJECTS::request &&req) {
	m_node->m_downloader.on_msg_notify_request_objects(this, req);
}

void Node::P2PClientCatalyst::on_disconnect(const std::string &ban_reason) {
...
void Node::P2PClientCatalyst::on_msg_notify_request_tx_pool(NOTIFY_REQUEST_TX_POOL::request &&req) {
...
void Node::P2PClientCatalyst::on_msg_timed_sync(COMMAND_TIMED_SYNC::request &&req) {
	m_node->m_downloader.on_msg_timed_sync(req.payload_data);
}
void Node::P2PClientCatalyst::on_msg_timed_sync(COMMAND_TIMED_SYNC::response &&req) {
	m_node->m_downloader.on_msg_timed_sync(req.payload_data);
}

void Node::P2PClientCatalyst::on_msg_notify_new_block(NOTIFY_NEW_BLOCK::request &&req) {
...
void Node::P2PClientCatalyst::on_msg_notify_new_transactions(NOTIFY_NEW_TRANSACTIONS::request &&req) {
...
void Node::P2PClientCatalyst::on_msg_network_state(COMMAND_REQUEST_NETWORK_STATE::request &&req) {
...
void Node::P2PClientCatalyst::on_msg_stat_info(COMMAND_REQUEST_STAT_INFO::request &&req) {
	if (!m_node->check_trust(req.tr)) {
		disconnect(std::string());
		return;
	}
...
void Node::advance_long_poll() {
	...
	if (m_long_poll_http_clients.empty())
		return;
	api::catalystd::GetStatus::Response resp = create_status_response3();
	...
	for (auto lit = m_long_poll_http_clients.begin(); lit != m_long_poll_http_clients.end();) {
		const bool method_status = lit->original_json_request.get_method() == api::catalystd::GetStatus::method() ||
		                           lit->original_json_request.get_method() == api::catalystd::GetStatus::method2();
		...
		if (method_status) {
			...
			try {
				api::catalystd::GetBlockTemplate::Request gbt_req;
				lit->original_json_request.load_params(gbt_req);
				api::catalystd::GetBlockTemplate::Response gbt_res;
...
</svg></td><td>catalystd &bull; version
)";
...
bool Node::on_api_http_request(http::Client *who, http::RequestData &&request, http::ResponseData &response) {
	...
	if (!m_config.catalystd_authorization.empty() &&
	    request.r.basic_authorization != m_config.catalystd_authorization) {
...
const std::unordered_map<std::string, Node::HTTPHandlerFunction> Node::m_http_handlers = {
    ...
    {api::catalystd::SyncBlocks::bin_method(), bin_method(&Node::on_wallet_sync3)},
    {api::catalystd::SyncMemPool::bin_method(), bin_method(&Node::on_sync_mempool3)},
...
const std::unordered_map<std::string, Node::JSONRPCHandlerFunction> Node::m_jsonrpc_handlers = {
    {api::catalystd::GetLastBlockHeaderLegacy::method(), json_rpc::make_member_method(&Node::on_get_last_block_header)},
    {api::catalystd::GetBlockHeaderByHashLegacy::method(),
        json_rpc::make_member_method(&Node::on_get_block_header_by_hash)},
    {api::catalystd::GetBlockHeaderByHeightLegacy::method(),
        json_rpc::make_member_method(&Node::on_get_block_header_by_height)},
    {api::catalystd::GetBlockTemplate::method(), json_rpc::make_member_method(&Node::on_getblocktemplate)},
    {api::catalystd::GetBlockTemplate::method_legacy(), json_rpc::make_member_method(&Node::on_getblocktemplate)},
    {api::catalystd::GetCurrencyId::method(), json_rpc::make_member_method(&Node::on_get_currency_id)},
    {api::catalystd::GetCurrencyId::method_legacy(), json_rpc::make_member_method(&Node::on_get_currency_id)},
    {api::catalystd::SubmitBlock::method(), json_rpc::make_member_method(&Node::on_submitblock)},
    {api::catalystd::SubmitBlockLegacy::method(), json_rpc::make_member_method(&Node::on_submitblock_legacy)},
    {api::catalystd::GetRandomOutputs::method(), json_rpc::make_member_method(&Node::on_get_random_outputs3)},
    {api::catalystd::GetStatus::method(), json_rpc::make_member_method(&Node::on_get_status3)},
    {api::catalystd::GetStatus::method2(), json_rpc::make_member_method(&Node::on_get_status3)},
...
bool Node::on_get_random_outputs3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::catalystd::GetRandomOutputs::Request &&request, api::catalystd::GetRandomOutputs::Response &response) {
...
api::catalystd::GetStatus::Response Node::create_status_response3() const {
	api::catalystd::GetStatus::Response res;
...
bool Node::on_get_status3(http::Client *who, http::RequestData &&raw_request, json_rpc::Request &&raw_js_request,
    api::catalystd::GetStatus::Request &&req, api::catalystd::GetStatus::Response &res) {
...
bool Node::on_sync_mempool3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::catalystd::SyncMemPool::Request &&req, api::catalystd::SyncMemPool::Response &res) {
...
bool Node::handle_send_transaction3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::catalystd::SendTransaction::Request &&request, api::catalystd::SendTransaction::Response &response) {
	NOTIFY_NEW_TRANSACTIONS::request msg;
	Transaction tx;
...
bool Node::handle_check_send_proof3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::catalystd::CheckSendProof::Request &&request, api::catalystd::CheckSendProof::Response &response) {
	Transaction tx;
	SendProof sp;
	try {
