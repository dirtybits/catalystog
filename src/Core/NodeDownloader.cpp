...
void Node::DownloaderV1::on_connect(P2PClientCatalyst *who) {
...
void Node::DownloaderV1::on_disconnect(P2PClientCatalyst *who) {
    if (who->is_incoming())
        return;
    m_good_clients.erase(who);
...
void Node::DownloaderV1::advance_download(Hash last_downloaded_block) {
    ...
    if (!m_sync_client) {  // start sync with some client who has better chain
        P2PClientCatalyst *worst_client = nullptr;
        for (auto &&who : m_good_clients) {
            if (!worst_client ||
...
void Node::DownloaderV1::on_msg_notify_request_chain(P2PClientCatalyst *who,
    const NOTIFY_RESPONSE_CHAIN_ENTRY::request &req) {
...
void Node::DownloaderV1::on_msg_notify_request_objects(P2PClientCatalyst *,
    const NOTIFY_RESPONSE_GET_OBJECTS::request &req) {
    if (!m_sync_sent)
        return;
